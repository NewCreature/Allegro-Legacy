...
static int import_bitmap_font_mono(BITMAP *import_bmp, FONT_GLYPH** gl, int num)
{
   ...
   for(i = 0; i < num; i++) {
	 ...
	 gl[i] = _AL_LEGACY_MALLOC(sizeof(FONT_GLYPH) + 8);
	 gl[i]->w = 8;
	 gl[i]->h = 8;
	 ...
	 gl[i] = _AL_LEGACY_MALLOC(sizeof(FONT_GLYPH) + sx * h);
	 gl[i]->w = w;
	 gl[i]->h = h;
...
FONT *grab_font_from_bitmap(BITMAP *bmp)
{
   ...
   f = _AL_LEGACY_MALLOC(sizeof *f);
   if (end == -1) end = bitmap_font_count(bmp) + begin;

   if (bitmap_font_ismono(bmp)) {
      FONT_MONO_DATA* mf = _AL_LEGACY_MALLOC(sizeof(FONT_MONO_DATA));

      mf->glyphs = _AL_LEGACY_MALLOC(sizeof(FONT_GLYPH*) * (end - begin));
	 ...
	 mf->next = NULL;
      ...
      FONT_COLOR_DATA* cf = _AL_LEGACY_MALLOC(sizeof(FONT_COLOR_DATA));
      cf->bitmaps = _AL_LEGACY_MALLOC(sizeof(BITMAP*) * (end - begin));
