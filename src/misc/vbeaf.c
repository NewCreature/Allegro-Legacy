...
static int load_vbeaf_driver(AL_LEGACY_CONST char *filename)
{
   ...
   af_driver = _accel_driver = _AL_LEGACY_MALLOC(size);

   if (pack_fread(af_driver, size, f) != size) {
      _AL_LEGACY_FREE(af_driver);
...
static GFX_MODE_LIST *vbeaf_fetch_mode_list(void)
{
   ...
   mode_info = _AL_LEGACY_MALLOC(sizeof(AF_MODE_INFO));
   if (!mode_info) return NULL;

   /* make sure VBE/AF interface is enabled! */
   if (!af_driver) {
      if (!vbeaf_locate_driver()) {
         if (mode_info) _AL_LEGACY_FREE(mode_info);
   ...
   gfx_mode_list = _AL_LEGACY_MALLOC(sizeof(GFX_MODE_LIST));
   if (!gfx_mode_list) {
      if (mode_info) _AL_LEGACY_FREE(mode_info);
   ...
   for (mode = af_driver->AvailableModes; *mode != 0xFFFF; mode++) {
      ...
      if (!gfx_mode_list->mode) {
         if (mode_info) _AL_LEGACY_FREE(mode_info);
         if (gfx_mode_list) _AL_LEGACY_FREE(gfx_mode_list);
         
         return NULL;
      }
      if (af_driver->GetVideoModeInfo(af_driver, *mode, mode_info) != 0) {
         if (mode_info) _AL_LEGACY_FREE(mode_info);
         if (gfx_mode_list->mode) _AL_LEGACY_FREE(gfx_mode_list->mode);
         if (gfx_mode_list) _AL_LEGACY_FREE(gfx_mode_list);
