...
void position_mouse_z(int z)
{
   ...
   _mouse_z = z;
   /* Directly update the z position here since the driver system doesn't
      handle it. Without this, Allegro 5 will overwrite the z position at the
      next MOUSE_AXES event. */
   al_set_mouse_z(z);
   update_mouse();
}
...
void position_mouse_w(int w)
{
   ...
   _mouse_w = w;
   /* Directly update the w position here since the driver system doesn't
      handle it. Without this, Allegro 5 will overwrite the w position at the
      next MOUSE_AXES event. */
   al_set_mouse_w(w);
   update_mouse();
}
