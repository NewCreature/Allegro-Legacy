...
{
   char *ext;
   FONT *(*load)(AL_LEGACY_CONST char *filename, RGB *pal, void *param);
 ...
 *  this format.
 */
void register_font_file_type(AL_LEGACY_CONST char *ext, FONT *(*load)(AL_LEGACY_CONST char *filename, RGB *pal, void *param))
{
   char tmp[32], *aext;
   ...
   if (!iter) 
      iter = font_type_list = _AL_LEGACY_MALLOC(sizeof(struct FONT_TYPE_INFO));
   else {
      for (iter = font_type_list; iter->next; iter = iter->next);
      iter = iter->next = _AL_LEGACY_MALLOC(sizeof(struct FONT_TYPE_INFO));
   }
 ...
 *  fails.
 */
FONT *load_font(AL_LEGACY_CONST char *filename, RGB *pal, void *param)
{
   char tmp[32], *aext;
   FONT_TYPE_INFO *iter;
   ASSERT(filename);
