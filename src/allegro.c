...
char allegro_id[] = "Allegro " ALLEGRO_LEGACY_VERSION_STR ", " ALLEGRO_LEGACY_PLATFORM_STR;
...
void al_assert(AL_CONST char *file, int line)
{
   ...
   if (assert_file) {
      ...
      if ((system_driver) && (system_driver->assert_buffer)) {
	 system_driver->assert_buffer(buf);
      }
      else {
	 allegro_exit();
	 fprintf(stderr, "%s\n", buf);
