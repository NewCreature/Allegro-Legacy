...
{
   AL_LEGACY_CONST char *unused = get_config_text("unused");
   int i, j, k;
...
int install_joystick(int type)
{
   ...
   for (c=0; driver_list[c].driver; c++) { 
	 ...
	 if (joystick_driver->init() != 0) {
	    if (!ugetc(allegro_error))
	       uszprintf(allegro_error, ALLEGRO_LEGACY_ERROR_SIZE, get_config_text("%s not found"), joystick_driver->name);
   ...
   if (!driver_list[c].driver) {
      ustrzcpy(allegro_error, ALLEGRO_LEGACY_ERROR_SIZE, get_config_text ("No joysticks found"));
      return -1;
   }
 ...
 */
int save_joystick_data(AL_LEGACY_CONST char *filename)
{
 ...
 *  save_joystick_data().
 */
int load_joystick_data(AL_LEGACY_CONST char *filename)
{
   char tmp1[64], tmp2[64];
   int ret, c;
 ...
 *  the specified stick.
 */
AL_LEGACY_CONST char *calibrate_joystick_name(int n)
{
