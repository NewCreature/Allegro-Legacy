...
static void blit_from_256(BITMAP *src, BITMAP *dest, int s_x, int s_y, int d_x, int d_y, int w, int h)
{
   ...
   if (_color_conv & COLORCONV_KEEP_TRANS) {
      dest_palette_color = _AL_LEGACY_MALLOC_ATOMIC(256*sizeof(int));
   ...
   if (_color_conv & COLORCONV_KEEP_TRANS)
      _AL_LEGACY_FREE(dest_palette_color);
...
static void dither_blit(BITMAP *src, BITMAP *dest, int s_x, int s_y, int d_x, int d_y, int w, int h)
{
   ...
   for (i=0; i<3; i++) {
      errline[i] = _AL_LEGACY_MALLOC_ATOMIC(sizeof(int) * w);
      errnextline[i] = _AL_LEGACY_MALLOC_ATOMIC(sizeof(int) * w);
   }
 ...
 getout:
   ...
   for (i=0; i<3; i++) {
      if (errline[i])
         _AL_LEGACY_FREE(errline[i]);

      if (errnextline[i])
         _AL_LEGACY_FREE(errnextline[i]);
   }
}
